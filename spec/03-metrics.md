...
**Brand traps (v3.1: heuristics nominate, one LLM call decides).** Apple seeds app names into the suggest index, so a phrase can carry an inflated P while existing only as some app's name. No SERP heuristic separates a seeded phantom ("gamblers recovery companion", P=63 off a 1-rating app) from generic search language that merely coincides with a weak app's name ("long distance relationship ldr" — a real query AND an exact tiny-app name; childCount separated nothing on labeled data). So detection is two-stage:

1. **Candidates** (heuristics, `difficulty.ts`): `isExactNameOfWeakApp` — the query IS the full normalized name of a weak (<200 ratings) top-3 app with no second full-name owner; fires at ANY sem, because a 1:1 name match is suspicious even for core-sounding phrases. Plus, at sem<3 only: the prescreen `brand` flag and `isBrandNameQuery` — the query matches a NAME SEGMENT (split on `:|–-·`) of a weak (<300) top-3 app with no ≥1000-rating full owner ("cbt thought record" → "TellMe: CBT Thought Record").
2. **Verdict** (LLM task `brandcheck`, one batched call per rating wave, pure COGS): "would real users type this phrase without knowing that app?" — with P, the matched app and the top SERP as evidence. `phantom:true` ⇒ `brandQuery`.

A confirmed trap is **excluded and not charged** (the user must never pay for a tombstone); its R stays computed and the reason carries the verdict (`… · brand trap: <reason>`).
