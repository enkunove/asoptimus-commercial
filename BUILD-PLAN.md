...
### D4 — Оплата ПО КЕЙФРАЗАМ, списание в РЕАЛЬНОМ ВРЕМЕНИ (usage-based) + внутренний COGS-учёт `[fix v4]`

**Решение пользователя (перекрывает прежний per-token-биллинг И прежний reserve/settle-вариант):**
usage-based — **платишь ровно за то, что использовано; кредиты списываются в реальном времени по
ходу прогона**. Единица = проверенная кейфраза из выборки. Пользователь ставит sampleSize (слайдер)
и модель — и СРАЗУ видит **оценку** стоимости. **1 кредит = $1. Free-tier НЕТ. Только пополнение
кредитов** (Stripe, $1/кредит, пакеты в конфиге).

- **Единица списания — проверенная кейфраза** (счётчик выборки spec 04.1: rated/selected/bench с
  R≥1). Цена = `pricePerKeyphrase[model]` кредитов; **мощнее модель → дороже кейфраза**
  (Haiku < Sonnet < Opus). Дефолт модели — **Haiku** (пользователь меняет в форме).
- **Списание в РЕАЛЬНОМ ВРЕМЕНИ, по факту:** как только кейворд становится проверенной кейфразой
  (rated, R≥1, входит в выборку) — **тут же атомарно списывается** `pricePerKeyphrase[model]`
  (`UPDATE wallet SET balance=balance-:p WHERE balance>=:p`), идемпотентно по `(run_id, keyword)`
  (`UNIQUE`, см. §5). Кредиты тают ЖИВЬЁМ по ходу прогона (виджет баланса — в реальном времени).
  Никакого reserve/settle/возврата: **сколько кейфраз произведено, столько и списано**.
- **Апфронт — это ОЦЕНКА, не удержание:** форма показывает `≈ sampleSize × pricePerKeyphrase[model]`
  живьём при слайдере/смене модели. Ремарка: «система может добить **до +10%** кейфраз (достраивает
  начатые ветки гипотез) — **они тоже списываются**, итог может быть до +10% выше оценки».
- **Overshoot ОПЛАЧИВАЕТСЯ** (это usage): произвели `sampleSize×1.1` → списано за все. Оркестратор
  кэпит перебор на `sampleSize × 1.1` (не разгоняется дальше), но что произведено — то оплачено.
  Ранняя остановка → списано только за произведённое (естественно — списание в реальном времени).
- **Hard-stop на нуле:** баланс не покрывает следующую кейфразу → прогон **paused** («кредиты
  кончились, пополни — продолжим с этого места»), резюмируемо. Конкурентные прогоны кошелька
  сериализуются на списании (атомарный `WHERE balance>=:p`) — в минус не уйти.

Источник истины по балансу — Postgres `wallet`. **НИЖЕ — ВНУТРЕННИЙ COGS-учёт** (нашими токенами):
он **не трогает кошелёк пользователя** (кошелёк списывается только по кейфразам, в реальном
времени); служит калибровкой `pricePerKeyphrase` и предохранителем. Механика:
- **Метрить КАЖДУЮ провайдер-попытку токенов** (не только успешную): логический LLM-шаг = до 6
  реальных billable-вызовов (`orchestrator.llm()` до 3× вокруг `completeJSON()` c 2× внутренними
  ретраями — `src/llm/claude.ts`; `trackUsage` пишет только последнюю успешную → недобор).
  llm-proxy пишет токены каждого `callOnce` в `llm_steps` (§5) — это НАШ реальный COGS, не чек юзеру.
- **`step_seq`** — серверный монотонный счётчик на billable-попытку; `UNIQUE(run_id, step_seq)`;
  результат шага персистится ДО advance, реплей читает его и **не зовёт Anthropic заново** (D7).
- **Цена токенов модели — из живого источника** (config/DB, не хардкод `PRICES_AS_OF`); сервер
  ходит в Anthropic по **api_key** (не subscription).
- **Калибровка + предохранитель:** сумма COGS прогона сравнивается с выручкой (списанные кейфразы
  × `pricePerKeyphrase`); если реальный COGS системно выше цены кейфразы — поднимаем цену; если в
  пределах прогона COGS вылетает за разумный потолок относительно уже списанного — предохранительная
  пауза. Маржа живёт в `pricePerKeyphrase` (кредит по номиналу $1).

(Митигации `ARCHITECTURE §4` — идемпотентность вебхуков, Radar — в силе. Idle-прогон на
`context_review` ничего не держит: списаний ещё не было — резервов нет.)
...
**Риски (по убыванию):**
1. **Экономика LLM + чарджбэки** — маржа в `pricePerKeyphrase`, real-time списание по кейфразам +
   hard-stop на нуле, внутренний per-attempt COGS-учёт + llm_steps (D4/D7), live-price, Radar.
...
**Решено (финал, после вопросов):**
- (D9) промпты в UI не показываются вообще.
- Биллинг — usage-based, списание в **реальном времени** по кейфразам (D4 v4); overshoot
  оплачивается; апфронт — только оценка. **1 кредит = $1, free-tier НЕТ, только top-up**.
- Модель прогона: выбор пользователя, **дефолт Haiku**; цена кейфразы растёт с мощностью модели.
- Лицензирование — **своё** (Postgres-ключи + session-token + HMAC), без Keygen/Paddle.
- Email — **свой сервис по SMTP** (провайдеро-агностичный: любой SMTP-relay; для доставляемости
  указать транзакционный SMTP, не самопальный mail-сервер — иначе спам).
- Приложение — **только macOS**, Tauri → **подписанный `.dmg`** (Developer ID + нотаризация; Apple
  Developer $99, сертификат вставляется в конце). Не App Store.
- Хостинг — **docker-compose** (сервер + Postgres), под Colima / любой Docker-хост.

**Открытые (уточняются цифрами в самом конце):** точные `pricePerKeyphrase[model]` и размеры
top-up-пакетов.
