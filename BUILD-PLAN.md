...
### D4 — Оплата ПО КЕЙФРАЗАМ выборки (предоплаченный квот) + внутренний COGS-учёт `[fix v3]`

**Решение пользователя (перекрывает прежний per-token-биллинг):** usage = **цена за проверенную
кейфразу из выборки**, не за токены. Пользователь ставит sampleSize (слайдер) и модель — и СРАЗУ,
ещё до старта, видит цену в кредитах. **1 кредит = $1. Free-tier НЕТ. Только пополнение кредитов**
(Stripe, $1/кредит, пакеты в конфиге).
...
- **Апфронт-квот (живьём при движении слайдера / смене модели):** `quote = ceil(sampleSize ×
  pricePerKeyphrase[model])`. Ремарка в UI: «система может добить до **+10%** кейфраз (достраивает
  начатые ветки гипотез) — включено в цену, доплаты нет».
- **Reserve на старте:** атомарно удержать `quote` (`UPDATE wallet SET balance=balance-:quote
  WHERE user_id=:u AND balance>=:quote`). Мало → прогон не стартует, «нужно ещё N кредитов,
  пополните». Конкурентные прогоны кошелька сериализуются (`FOR UPDATE`).
- **Settle в конце:** списать за фактически доставленные кейфразы, но НЕ больше квота (overshoot
  до +10% поглощаем): `charge = min(delivered, sampleSize) × pricePerKeyphrase`; вернуть
  `quote − charge`. Ранняя остановка / нашлось меньше → платит за доставленное, остаток назад.
  Идемпотентно по `run_id`.
- **Пайплайн-констрейнт под «+10%»:** оркестратор перестаёт заводить новые ветки при подходе
  выборки к `sampleSize × 1.1` — иначе обещание «до +10%» врёт.

Источник истины по балансу — Postgres `wallet`. **НИЖЕ — ВНУТРЕННИЙ COGS-учёт** (нашими токенами):
он НЕ то, что платит пользователь; служит калибровкой `pricePerKeyphrase` и предохранителем
(реальный COGS вылез за квот → пауза). Механика:
...
- **Цена модели — из живого источника (config/DB), не из хардкод-таблицы адаптера**
  ...
  бесплатные прогоны.
- Маржа — **в `pricePerKeyphrase`** (кредит по номиналу $1), НЕ на покупке. Внутренний per-attempt
  COGS сравнивается с выручкой (кейфразы × цена) — мониторинг маржи по моделям, не чек юзеру.
...
**Риски (по убыванию):**
1. **Экономика LLM + чарджбэки** — маржа в `pricePerKeyphrase`, апфронт-reserve квота, внутренний
   per-attempt COGS-учёт + `UNIQUE(run_id,step_seq)` + llm_steps (D4/D7), live-price, Radar.
...
- Биллинг — per-keyphrase квот (D4 v3): **1 кредит = $1, free-tier НЕТ, только top-up**.
